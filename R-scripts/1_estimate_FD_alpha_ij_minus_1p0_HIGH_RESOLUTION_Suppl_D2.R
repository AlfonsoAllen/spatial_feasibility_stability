...
epsilon <- 0.005
r1A <- seq(0.055-epsilon, 0.055+epsilon, by = 0.00001) # seq(-0.1, 0.1, by = 0.005) 
r2A <- seq(0.08-epsilon, 0.08+epsilon, by = 0.00001)
r1B_aux = 0.08000
r2B_aux = 0.05500

# x <- seq(-0.2, 0.2, by = 0.00002)
# length(x)^2/1e6
...
for(D_i in 1:length(D_vector)){
  ...
  data_growth_rates <- expand.grid(r1A = r1A, 
                                       r1B = r1B_aux, 
                                       r2A = r2A, 
                                       r2B = r2B_aux, 
                                       D = D_vector[D_i])
  ...
  readr::write_csv(data_growth_rates,paste0("Results/FD_2sp_toy_DOM_ZERO_V5_",(1/D_vector[D_i]),"xD_EXAMPLE_V2.csv"))
